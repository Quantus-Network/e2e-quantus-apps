...
class _WithdrawalScreenState extends State<WithdrawalScreen> {
  ...
  Future<void> _initAddressManager() async {
    try {
      ...
      _log.i('Address manager initialized with ${_addressManager.allAddresses.length} addresses');
    } catch (e) {
  ...
  Future<void> _loadTrackedTransfers() async {
    try {
      ...
      _transferTrackingService.initialize(rpcUrl: chainConfig.rpcUrl, wormholeAddresses: addressesToTrack);
      ...
      final primaryTransfers = await _transferTrackingService.getUnspentTransfers(
        wormholeAddress: widget.wormholeAddress,
        secretHex: widget.secretHex,
      );
      allTransfers.addAll(primaryTransfers);
      _log.i('Primary address ${widget.wormholeAddress}: ${primaryTransfers.length} unspent');
      ...
      for (final tracked in _addressManager.allAddresses) {
        ...
        if (transfers.isNotEmpty) {
          ...
          _log.i('Change address ${tracked.address}: ${transfers.length} unspent');
        }
      }
