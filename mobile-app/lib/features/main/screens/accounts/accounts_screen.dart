...
class _AccountsScreenState extends ConsumerState<AccountsScreen> {
  ...
  Widget _buildAccountListItem(Account account, bool isActive, int index) {
    final entrustedAccounts = [
      TreeNode(
        data: Account(index: 0, name: 'hehe', accountId: 'dwadwadwadwa'),
      ),
      TreeNode(
        data: Account(index: 0, name: 'hehe', accountId: 'dwadwadwadwa'),
      ),
    ];
    // final entrustedAccounts = account.entrustedAccounts
    //     .map((entrusted) => TreeNode<Account>(data: entrusted))
    //     .toList();

    // We calculate how many space we will give to the tree view according to the
    // number of nodes we have. Each item take 52. The max is 104 for the height so we don't give
    // to much space that make UI looks ugly
    final double constraintMaxHeight = min(entrustedAccounts.length * 52, 104);
